...
//      azure_key = my_key.c_str();
    ...
    irods::error azureFileUnlinkPlugin( irods::plugin_context& _ctx ) {
        ...
        try {
           ...
           // hashed
           rodsLog(LOG_DEBUG6, "physical_path in unlink %s", file_obj->physical_path().c_str());
           rodsLog(LOG_DEBUG6, "logical_path in unlink  %s", file_obj->logical_path().c_str());
           ...
           status = deleteTheFile(container.c_str(), fileName.c_str(), connection.c_str());
    ...
    irods::error azureFileStatPlugin(
        ...
        try {
           ...
           rodsLog(LOG_DEBUG6, "physical_path in FileStat %s", file_obj->physical_path().c_str());
           rodsLog(LOG_DEBUG6, "logical_path in FileStat  %s", file_obj->logical_path().c_str());
           std::string container = getHashedContainerName(file_obj->physical_path());
           ...
           status = getTheFileLength(container.c_str(), file_name.c_str(), connection.c_str(), &len);
           ...
           if (status) {
                ...
                rodsLog(LOG_DEBUG6, "azureFileStatPlugin - error in getTheFileLength");
                result =  ERROR( status, "azureFileStatPlugin - error in getTheFileLength");
              } 
           } catch ( const irods::exception& e) {
              irods::log( irods::error(e) );
    ...
    irods::error azureStageToCachePlugin(
        ...
        try {
           ...
           irods::file_object_ptr file_obj = boost::dynamic_pointer_cast< irods::file_object >( _ctx.fco() );
           rodsLog(LOG_DEBUG6, "physical_path in StageToCache %s", file_obj->physical_path().c_str());
           rodsLog(LOG_DEBUG6, "logical_path in StageToCache %s", file_obj->logical_path().c_str());
           ...
           bool status = getTheFile(container.c_str(), 
                                   file_name.c_str(),
                                   connection.c_str(), 
                                   _cache_file_name, 
                                   file_obj->mode()); 
           if(!status) {
               result =  ERROR( UNIV_MSS_STAGETOCACHE_ERR, "azureStageToCachePlugin - error in getTheFile");
           }
        } catch ( const irods::exception& e) {
           irods::log( irods::error(e) );
           result = ERROR( SYS_INVALID_INPUT_PARAM, e.what());
    ...
    irods::error azureSyncToArchPlugin( 
        ...
        try {
           ...
           // the hashed container name is the hash of the physical path not including the file name.
           rodsLog(LOG_DEBUG6, "physical_path in SyncToArch %s", file_obj->physical_path().c_str());
           rodsLog(LOG_DEBUG6, "logical_path in SyncToArch  %s", file_obj->logical_path().c_str());
           ...
           wrapper_result = putTheFile( connection.c_str(),
                                (const char *)_cache_file_name, 
                                file_name.c_str(),
           ...
           if (wrapper_result) {
              ...
              result = ERROR( code, std::string("azureSyncToArchPlugin - error in putTheFile" ));
           }
        } catch ( const irods::exception& e) {
           irods::log( irods::error(e) );
    ...
    irods::error register_archive_object(
        ...
        try {
           ...
           const utility::string_t connection = createConnectionString(_ctx);
           rodsLog(LOG_DEBUG6, "physical_path in register %s", _file_obj->physical_path().c_str());
           rodsLog(LOG_DEBUG6, "logical_path in register  %s", _file_obj->logical_path().c_str());

           std::string container = getHashedContainerName(_file_obj->logical_path());
           std::string file_name = getFileName(_file_obj->logical_path());

           rodsLog(LOG_DEBUG6, "container in register %s", container.c_str());
           rodsLog(LOG_DEBUG6, "file_name in register  %s", file_name.c_str());
           ...
           status = getTheFileStatus( container.c_str(), file_name.c_str(), connection.c_str() );
           if ( status ) {
