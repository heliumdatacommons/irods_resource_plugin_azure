...
class Test_Compound_with_AZURE_Resource(resource_suite.ResourceSuite, ChunkyDevTest, unittest.TestCase):
    def setUp(self):
        ...
        with session.make_session_for_existing_admin() as admin_session:
            ...
            admin_session.assert_icommand('iadmin mkresc archiveResc azure '+hostname+':/empty/path "AZURE_ACCOUNT_FILE=/etc/irods/azure_account.txt"', 'STDOUT_SINGLELINE', 'archiveResc')
        ...
        super(Test_Compound_with_AZURE_Resource, self).setUp()

    def tearDown(self):
        super(Test_Compound_with_AZURE_Resource, self).tearDown()
        with session.make_session_for_existing_admin() as admin_session:
    ...
    def test_retry_for_put(self):
        # set up
        self.admin.assert_icommand("iadmin modresc archiveResc context AZURE_ACCOUNT_FILE=XXXX;retry_count=2;connect_timeout=5")
        filename = "some_test_file.txt"
        filepath = lib.create_local_testfile(filename)

        # test it
        self.admin.assert_icommand( "iput -f "+filepath, 'STDERR_SINGLELINE', "UNIV_MSS_SYNCTOARCH_ERR")
